...
class DefaultController extends ControllerBase {
  ...
  /** @var \Drupal\custom_model\Services\CustomModelGlobalfunction */
...
//$output = theme_table($pending_header, $pending_rows);
  ...
  public function dwsim_custom_model_uploaded_tab() {
    ...
    if (empty($result)) {
      $page_content['message'] = [
        '#markup' => '<p>Uploaded Proposals under Custom Model Project: 0</p>',
      ];
    }
    else {
      $proposal_rows = [];
...
public function custom_model_abstract() {
  ...
  public function custom_model_download_completed_project() {
    ...
    if ($zip_file_count > 0) {
      ...
      $this->messenger->addError('There are no custom model project files in this proposal to download.');
    }
  }
...
public function custom_model_download_full_project() {
  ...
  public function custom_model_progress_all() {
    ...
    $page_content = [
      '#markup' => '<p>Work is in progress for the following custom models.</p>',
    ];
    if ($preference_rows) {
      $page_content = [
        '#type' => 'table',
  ...
  public function _list_custom_model_certificates() {
    ...
    if ($exist_id && $exist_id->id) {
      if ($exist_id->id < 3) {
        $this->messenger->addStatus('<strong>You need to propose a <a href="https://dwsim.fossee.in/custom-model/proposal">Custom Model Project</a></strong>. If you have already proposed then your Custom Model is under the reviewing process');
        return '';
      }
      $search_rows = [];
    ...
    $this->messenger->addStatus('<strong>You need to propose a <a href="https://dwsim.fossee.in/custom-model/proposal">Custom Model Project</a></strong>. If you have already proposed then your Custom Model is under the reviewing process');
    return "<span style='color:red;'> No certificate available </span>";
  }
...
public function custom_model_progress_all() {
  ...
  public function _list_custom_model_certificates() {
    ...
    if ($exist_id && $exist_id->id) {
      if ($exist_id->id < 3) {
        $this->messenger->addStatus('<strong>You need to propose a <a href="https://dwsim.fossee.in/custom-model/proposal">Custom Model Project</a></strong>. If you have already proposed then your Custom Model is under reviewing process');
        return ['#markup' => ''];
      }
      $search_rows = [];
      $query3 = $this->connection->query("SELECT id, project_title, contributor_name FROM custom_model_proposal WHERE approval_status=3 AND uid= :uid", [
      ...
      while ($search_data3 = $query3->fetchObject()) {
        if ($search_data3->id) {
          $cert_link = Link::fromTextAndUrl(
            $this->t('Download Certificate'),
            Url::fromUri('internal:/custom-model/certificates/generate-pdf/' . $search_data3->id)
          )->toString();
          $search_rows[] = [
            ['data' => ['#markup' => $search_data3->project_title]],
            ['data' => ['#markup' => $search_data3->contributor_name]],
            ['data' => ['#markup' => $cert_link]],
          ];
        }
      }
      if ($search_rows) {
      ...
      return ['#markup' => $this->t('No certificates available.')];
    }
    $this->messenger->addStatus('<strong>You need to propose a <a href="https://dwsim.fossee.in/custom-model/proposal">Custom Model Project</a></strong>. If you have already proposed then your Custom Model is under reviewing process');
    return ['#markup' => "<span style='color:red;'> No certificate available </span>"];
  }
